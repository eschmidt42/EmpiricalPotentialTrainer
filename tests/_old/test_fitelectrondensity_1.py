...
pure Python with active self contribution. Testing isotropic only.
"""
...
class DensityRegressionFixture(unittest.TestCase):
    def setUp(self):
        ...
        # writing paths
        self.load_path_rhos = pre_path+"/unittest_files/test_density_withself.rhos"
        #save_path_predicted = "./predicted_Ni3Al_ideal_dis.pred"
        self.load_path_Phi = pre_path+"/unittest_files/test_density_withself.phi"
        ...
        if self.stochastic[0] == "all":
            ...
            # get design matrix
            #self.Phi, self.t = fed.rvm_old.get_design_matrix(self.bonds,self.basis,self.mapper,verbose=True,
            #        return_t=True,seed=self.seed,usefortran=False)
...
class Test_fitelectrondensity(DensityRegressionFixture):
    ...
    def test2_optimizing_hyperparameters(self):
        ...
        logbook = fed.rvm.iterate(self.Phi,self.t,niter=self.niter,verbose=False,alpha_init=alpha_init,
            beta_init=beta_init,tol=self.tol, fix_beta=self.fix_beta,sequential=self.sequential,
            n_steps_beta=self.n_steps_beta,max_memory=self.max_memory,seed=self.seed)
