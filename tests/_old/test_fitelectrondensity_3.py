...
pure Python with active self contribution. Testing anisotropic.
"""
    ...
    def setUp(self):
        ...
        for tmp_gip in self.gip:
           ...
           tmp_bonds = fed.misc.get_observations(tmp_gip,ultra_num=self.ultra_num,num_neigh=self.num_neigh,
                ...
                seed=self.seed,ani_type=self.ani_type,ani_specification=self.ani_specification,
...
class Test_fitelectrondensity(DensityRegressionFixture):
    ...
    def test0_reproducing_obs(self):
        ...
        for i in range(Nbonds):
            if i<0:
                print("\nbonds {}".format(tuple(self.bonds[i].x["ani"].keys())))
                print("bonds {}".format(self.bonds[i].x))
                print("_bonds {}".format(tuple(_bonds[i]["ani"].keys())))
