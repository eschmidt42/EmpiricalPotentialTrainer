...
from fitenergy.misc_serial import setup_weights
from fitenergy.potential_serial import calculate_all_energies, calculate_all_forces, calculate_all_forces_from_splines

def log_joint(x,t,alphas,betas,emb_map,pair_map,emb_neigh_map,params,emb_n,emb_i,pair_n,negative=True,force_analytic=True):
        ...
        emb_map,pair_map,emb_neigh_map : output of misc_serial.get_mappers
        
        params: parameters calculated in EAM_setup
        
        emb_n, emb_i, pair_n : output of misc_serial.setup_emb_and_pair_contribution            
        ...
        logp : float
            log p(X,Z|theta) or - log p(X,Z|theta) depending on parameter "negative"
        """
        tmp_x = np.array(x,dtype=float)
            
        W_emb,W_pair,W_emb_neigh = setup_weights(tmp_x,emb_map,pair_map,emb_neigh_map,params) #fe.nonlin.setup_weights(tmp_x,params,mapper)
        W_pair_neigh = W_pair
        
        energies = calculate_all_energies(W_emb,W_pair,params)
        
        if force_analytic:
            forces = calculate_all_forces(W_emb,W_pair_neigh,W_emb_neigh,params,emb_n,emb_i,pair_n)
        else:
...
def least_square_measure(x,emb_map,pair_map,emb_neigh_map,data,emb_n,emb_i,pair_n,t=None):
    """
    Least square measure provided the references, the functions and the weights.
    Energies and forces are being calculated.
    """
    tmp_x = np.array(x,dtype=float)
    W_emb,W_pair,W_emb_neigh = setup_weights(tmp_x,emb_map,pair_map,emb_neigh_map,data["params"])#fe.nonlin.setup_weights(tmp_x,params,mapper)
    W_pair_neigh = W_pair
    energies = calculate_all_energies(tmp_x,W_emb,W_pair)
    if force_analytic:
        forces = calculate_all_forces(tmp_x,W_emb,W_pair_neigh,W_emb_neigh,data["params"],emb_n,emb_i,pair_n)
    else:
        forces = calculate_all_forces_from_splines(tmp_x,data)
    N = len(t["forces"])
    M = len(x)
...
class calculator:
    ...
    def __call__(self,x):
        ...
        if self.evl_fun.__name__ == "log_joint":
            ...
            emb_map = self.params["emb_map"]
            pair_map = self.params["pair_map"]
            emb_neigh_map = self.params["emb_neigh_map"]
            emb_n = self.params["emb_n"]
            emb_i = self.params["emb_i"]
            pair_n = self.params["pair_n"]
            val = self.evl_fun(x,self.t,alphas,betas,emb_map,pair_map,emb_neigh_map,params,emb_n,emb_i,pair_n,negative=True,force_analytic=True)
            ...
            emb_i = self.params["emb_i"]
            pair_n = self.params["pair_n"]
            val = self.evl_fun(x,emb_map,pair_map,emb_neigh_map,data,emb_n,emb_i,pair_n,self.t)
...
def wrapper_bayes_noise(x,t,alphas,emb_map,pair_map,emb_neigh_map,params,emb_n,emb_i,pair_n,force_analytic=True):
    
    alphas = np.absolute(alphas)
    tmp_x = np.array(x,dtype=float)
    W_emb,W_pair,W_emb_neigh = setup_weights(tmp_x,emb_map,pair_map,emb_neigh_map,params)
    W_pair_neigh = W_pair
    energies = calculate_all_energies(W_emb,W_pair,params)
...
def wrapper_bayes_regularizer(x,t,betas):
